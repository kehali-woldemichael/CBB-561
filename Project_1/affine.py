### This file contains source code for Needleman-Wunsch Algorithm with affine scoring
### written by Qi Gao

from read_fasta_and_weight import return_sc

def Needleman_Wunsch_affine(seq1, seq2, substitution_scores, gap_open_penalty = 4, gap_extend_penalty = 0.1):
    l1 = len(seq1)
    l2 = len(seq2)
    ...
    for i in range(l2):
        for j in range(l1):
            ...
            if T[i][j + 1] == 1:
                cand.append(F[i][j + 1] - gap_extend_penalty)
            else:
                cand.append(F[i][j + 1] - gap_open_penalty)
            if T[i + 1][j] == 2:
                cand.append(F[i + 1][j] - gap_extend_penalty)
            else:
                cand.append(F[i + 1][j] - gap_open_penalty)
